...
# Script to delete a git worktree
# Usage: delete-worktree.sh [branch-name]
...
if [ -n "$1" ]; then
    ...
    if [ "$CURRENT_TOPLEVEL" = "$PROJECT_DIR" ]; then
        echo "Error: You are in the main repository, not a worktree. Specify branch name to delete."
        echo "Usage: $0 <branch-name>"
        exit 1
    fi
    if git -C "$PROJECT_DIR" worktree list | grep -q "$CURRENT_TOPLEVEL"; then
        WORKTREE_DIR="$CURRENT_TOPLEVEL"
    else
        echo "Error: Current directory is not a worktree of $PROJECT_DIR. Specify branch name."
        echo "Usage: $0 <branch-name>"
        exit 1
    fi
fi
