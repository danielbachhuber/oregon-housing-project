...
const EXEMPLAR_SLUGS = ['sightline-institute', 'oregon-housing-and-community-services'];
...
function loadExemplarProfiles(): string {
  ...
  for (const slug of EXEMPLAR_SLUGS) {
    const filePath = path.join(CONTENT_DIR, 'organizations', `${slug}.md`);
...
async function main() {
  ...
  if (!slug) {
    console.error('Usage: pnpm refine-organization <slug> [--dry-run]');
    console.error('  e.g. pnpm refine-organization sightline-institute');
    console.error('  e.g. pnpm refine-organization sightline-institute --dry-run');
    process.exit(1);
  }
  ...
  const template = fs.readFileSync(path.join(TEMPLATES_DIR, 'organization.md'), 'utf-8');
...
${exemplarProfiles}`;
  ...
  // Build user prompt
  const orgName = isExisting
    ? (extractTitle(filePath) || slugToName(slug))
    : slugToName(slug);

  let userPrompt: string;
  if (isExisting) {
    userPrompt = `Refine and improve this existing profile for ${orgName}. Keep any accurate information and citations, but improve the structure, add missing context, ensure completeness per the template, and fill in any gaps.
...
${existingContent}`;
    ...
    userPrompt = `Create a new profile for ${orgName}. Research this organization's role in Oregon housing policy and write a complete profile following the template instructions.
